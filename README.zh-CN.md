...
| 1. 添加视频 | 2. 智能挑帧，再微调 | 3. 生成拼图 | 4. 本地导出 |
| --- | --- | --- | --- |
| 选择一个视频，或一次加入多个视频。 | 识别转场，避开黑屏、模糊与重复画面；需要时浏览候选画面并重新选帧。 | 3×3 至 8×8 网格逐张实时预览。 | 在视频同目录保存宽度至少 1920 px 的 PNG 或 JPG。 |
...
- 生成后可浏览新一组候选画面，智能选择并按需微调，再按原结果的输出设置另存新图。
